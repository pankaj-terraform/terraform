...
module "create_dynamodb_table" {
  ...
  #Tags
  default_tags = merge(map("application_id",local.application_id),map("environment",local.environment),
			map("created_by",local.created_by),map("stack_name",local.stack_name),
			map("termination_date",local.termination_date),map("date_class",local.date_class))
}
